...
``` r
str(iris) # display the internal structure of an R obj
```
...
``` r
colnames(iris) # set the row or column names of a matrix-like object
```
...
``` r
head(iris) # shows the first parts of a vesctor, matix, table, data frame func
```
...
``` r
tail(iris) # shows the last parts of a vesctor, matix, table, data frame func
```
...
``` r
iris$Sepal.Length # ekstraksi kolom 'Sepal.Length' dari dataset iris. Jalankan help(`[`) untuk membaca dokumentasi. operators acting on vectors, matrices, arrays and lists to extract or replace parts
```
...
``` r
class(iris$Species) # mengklasifikasi
```

    ## [1] "factor"

``` r
length(iris$Petal.Width) # ukuran
```

    ## [1] 150

``` r
levels(iris$Species) # melihat nilai secara level
```
...
``` r
skim(typical_data) # harus pake package stingi
```
