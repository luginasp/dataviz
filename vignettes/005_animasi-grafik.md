...
remotes::install_github('cttobin/ggthemr')
library(ggthemr)
# Dipasang dengan cara menjalankan kode: remotes::install_github('cttobin/ggthemr')
...
``` r
library(gifski) #jgn lupa install png
plot_temperature +
  transition_reveal(time) #garis muncul perlahan
```
...
plot_weather <- ggplot(weather, aes(x = temperature, y = pressure, colour = location, size = windspeed)) +
  geom_point()
plot_weather #buat ngesummon visual(?)
```
...
animate_weather <- 
  plot_weather  +
  transition_states(location) + #munculnya animasi scatter plot akan berdasarkan lokasi
...
animate_weather <- 
  ...
  labs(
    x = "suhu (C)",
    y = "tekanan",
    title = "hubungan suhu dan tekanan",
    subtitle = "Lokasi pengamatan: {closest_state}",
    caption = "Grafik ini dibuat oleh luginasp",
    size = "windspeed (mps)"
  )
animate_weather
```
