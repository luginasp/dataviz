...
gdp_id %>% 
  ...
  geom_point(alpha = 0.5) # pokoknya isi berdasarkan nama variabelnya
```
...
ggplot(jabar_road, aes(x = district, y = road_length, fill = condition)) +
  ...
  theme(axis.text = element_text(angle = 30)) # buat ngedit posisi label pada sumbu
```
...
```
    ...
    ## Reading layer `states_provinces' from data source `C:\Users\Students\Desktop\w2\dataviz\data-raw\states_provinces.json' using driver `GeoJSON'
